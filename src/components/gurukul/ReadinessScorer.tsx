...
const BANDS = [
  {
    min: 0, name: "AI-Distant", color: "#c2410c",
  ...
  {
    min: 26, name: "AI-Curious", color: "#b45309",
  ...
  {
    min: 51, name: "AI-Emerging", color: "#0f766e",
...
export default function ReadinessScorer() {
  ...
  if (submitted) {
    return (
      <div className="grid gap-6">
        {/* score card */}
        <section className="border-2 border-[#1b2026] bg-white p-6 sm:p-8 text-center">
          <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-[#9b9588] mb-3">Your school's AI-readiness score</p>
          <p className="font-display text-7xl sm:text-8xl font-bold tracking-[-0.04em] text-[#15191e] leading-none">
          ...
          <p className="mt-5 text-[15px] leading-[1.7] text-[#3a4048] max-w-[58ch] mx-auto text-left sm:text-center">
            {result.band.verdict}
          </p>
        </section>

        {/* dimension breakdown */}
        <section className="border-2 border-[#1b2026] bg-white p-5 sm:p-6">
          ...
          <div className="grid gap-3">
            {result.dims.map((d) => (
              <div key={d.dim} className="grid grid-cols-[140px_1fr_36px] sm:grid-cols-[180px_1fr_44px] items-center gap-3">
                <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[#3a4048] truncate">{d.dim}</span>
                <span className="h-2.5 bg-[#1b2026]/10">
                  <span className="block h-full transition-all duration-700" style={{ width: `${(d.pts / 3) * 100}%`, background: d.pts <= 1 ? "#c2410c" : d.pts === 2 ? "#b45309" : "#0f766e" }} />
                </span>
                <span className="font-mono text-[11px] text-[#3a4048] text-right">{d.pts}/3</span>
              </div>
            ))}
          </div>
        </section>
          ...
          <ol className="grid gap-4">
            {result.weakest.map((d, i) => (
              <li key={d.dim} className="flex gap-4">
                <span className="shrink-0 w-7 h-7 grid place-items-center border-2 border-[#1b2026] font-mono text-[12px] font-bold text-[#15191e]">{i + 1}</span>
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#e8650a] mb-1">{d.dim}</p>
                  <p className="text-[14px] leading-[1.7] text-[#3a4048]">{d.fix}</p>
                </div>
              </li>
            ))}
          </ol>
        ...
        {/* CTAs */}
        <section className="border-2 border-[#1b2026] bg-[#1b2026] text-white p-6 sm:p-7 print:hidden">
          <p className="font-display text-xl sm:text-2xl font-semibold tracking-[-0.01em] mb-2">
          ...
          <div className="flex flex-wrap gap-3">
            <a href={CAL_URL} target="_blank" rel="noopener noreferrer" className="bg-[#e8650a] hover:bg-[#c75100] transition-colors text-white font-mono text-[11px] uppercase tracking-[0.18em] px-6 py-3">
          ...
          <div className="flex flex-wrap gap-4 mt-5 pt-4 border-t border-white/15">
            <a href={waShare} target="_blank" rel="noopener noreferrer" className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#7dd87d] hover:text-white transition-colors">
  ...
  return (
    <div className="grid gap-5">
      {/* progress */}
      <div className="sticky top-0 z-10 -mx-4 sm:mx-0 px-4 sm:px-0 py-3 bg-[#f3eee3]/95 backdrop-blur-sm">
        ...
        <div className="h-1.5 bg-[#1b2026]/10">
          <div className="h-full bg-[#e8650a] transition-all duration-300" style={{ width: `${(answered / QUESTIONS.length) * 100}%` }} />
        </div>
      </div>

      {QUESTIONS.map((q, qi) => (
        <section key={qi} className="border-2 border-[#1b2026] bg-white p-5">
          <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#e8650a] mb-1.5">{String(qi + 1).padStart(2, "0")} · {q.dim}</p>
          <h2 className="text-[15.5px] font-semibold text-[#15191e] leading-snug mb-3">{q.q}</h2>
              ...
              return (
                <button
                  ...
                  className={`text-left border-2 px-3.5 py-2.5 text-[13.5px] leading-snug transition-colors ${
                    picked ? "border-[#e8650a] bg-[#e8650a]/8 text-[#15191e]" : "border-[#1b2026]/20 hover:border-[#e8650a] text-[#3a4048]"
                  }`}
                >
                  {opt}
                </button>
      ...
      <button
        ...
        className="bg-[#e8650a] disabled:bg-[#1b2026]/20 disabled:cursor-not-allowed text-white font-mono text-[12px] uppercase tracking-[0.2em] px-8 py-4 hover:bg-[#c75100] transition-colors"
      >
